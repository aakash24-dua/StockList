...
class ContentItemViewHolder(val root: View) : RecyclerView.ViewHolder(root) {
    ...
    fun bind(response: Searchresult?) {
        ...
        val maxValue = response.high?.toDouble()?.minus(response?.low?.toDouble()!!)?.times(100)?.toInt()!!
        val progress = response.current?.toDouble()?.minus(response?.low?.toDouble()!!)?.times(100)?.toInt()
